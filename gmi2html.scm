...
(define (sxml-rules directory input-filename)
  ...
  (define (a _ attrs . text)
    (let* ((attrs (cdr attrs))
           ...
           (text (if (and (not (null? text))
                          (null? (cdr text))
                          (string? (car text))
                          (string=? (car text) uri))
                   #f
                   text)))
      (receive (new-uri image?) (rewrite-uri directory input-filename uri)
        (let ((text (or text `(,new-uri))))
          ((if image? make-img-tag make-a-tag) new-uri attrs text)))))
